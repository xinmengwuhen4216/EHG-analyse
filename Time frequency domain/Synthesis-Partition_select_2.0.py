...
def pred_svc(X_train, X_test, y_train, y_test):
    ...
    return sensitivity, specificity, Gmean, Area, Accuracy
...
for p in range(2, 151):
    ...
    list_Area = []
    list_Accuracy = []
    smote_list = np.array(Smote_Algorithm(features))
    kf = KFold(n_splits=5)
    # 进行经过特诊选择后进行10次五重折叠交叉验证
    for i in range(200):
        sensitivity_list = []
        specificity_list = []
        Gmean_list = []
        ...
        for train_index, test_index in kf.split(smote_list):
            train_list = np.array(smote_list)[train_index]
            test_list = np.array(smote_list)[test_index]
            ...
            # 使用SVM进行早产预测
            sensitivity, specificity, Gmean, Area, Accuracy = pred_svc(train_X_1, test_X_1, train_Y, test_Y)
...
print("clustering_sensitivity=", final_sensitivity)
print("clustering_specificity=", final_specificity)
print("clustering_Gmean=", final_Gmean)
# print("Slop_F_measure=", final_F_measure)
print("clustering_Area=", final_Area)
