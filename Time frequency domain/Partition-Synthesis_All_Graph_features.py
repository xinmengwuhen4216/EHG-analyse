...
for each in L_slop:
    feature_all_slop = np.abs(list(map(float, each)))
...
for each in L_assortativity:
    feature_all_assortativity = np.abs(list(map(float, each)))
...
for each in L_clustering:
    feature_all_clustering = np.abs(list(map(float, each)))
    feature_to_float_clustering.append(feature_all_clustering)
float_slop = SelectKBest(chi2, k=38).fit_transform(np.array(feature_to_float_slop)[:, :-1], np.array(feature_to_float_slop)[:, -1])
float_assortativity= SelectKBest(chi2, k=31).fit_transform(np.array(feature_to_float_assortativity)[:, :-1], np.array(feature_to_float_assortativity)[:, -1])
float_clustering = SelectKBest(chi2, k=24).fit_transform(np.array(feature_to_float_clustering)[:, :-1], np.array(feature_to_float_clustering)[:, -1])
Graph_features = np.hstack(
    (np.hstack((np.array(float_slop),
                np.array(float_assortativity))),
               np.array(float_clustering)))
final_sensitivity = []
final_specificity = []
final_Gmean = []
final_F_measure = []
...
for p in range(2, 93):
    print("P:",p)
    X_new = SelectKBest(chi2, k=p).fit_transform(Graph_features, np.array(feature_to_float_slop)[:, -1])
